...
import {
  ...
  type StopCommand,
  type Coordinate,
  type LadybugLanguage,
  type LadybugCommand,
} from "..";
import grammar from "./ladybug.ohm-bundle";

const semantics = grammar.createSemantics();
const ladybugDrawing = {
  check,
  evaluate,
} satisfies LadybugLanguage;

function evaluate(expr: string): LadybugCommand[] {
...
semantics.addOperation<LadybugCommand>("command()", {
  drawToCommand(x) {
    return x.command();
  },
...
semantics.addOperation<LadybugCommand[]>("run()", {
